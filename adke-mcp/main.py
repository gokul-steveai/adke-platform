...
async def lifespan(app: FastAPI):
    ...
    global supabase_client
    supabase_client = await SupabaseService.get_client()
    yield
    ...
    if response is None:  # Handle the case where the agent fails to produce a response
        ...
        if response.get("intermediate_steps"):
            ...
            if action.tool == "execute_safe_command":
                ...
                db_response = await supabase_client.table("incident_logs").insert({
                    "action_type": "CLI_EXECUTION",
...
async def reject_action(action_request: dict):
